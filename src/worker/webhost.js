				...
				hostDomain.run(function() {
					hostsLayout[hostKey].forEach(function(config) {
						...
						try {
							...
							Object.defineProperty(handlerconstants, "comm", {
								get: function() {
									throw "Comm not implemented yet";
								}
							});
		...
		if(host.configured) {
			...
			if(host.configured instanceof Error)
				router = function(req, res, next) {
					host.logger.warning("Sending error");
			...
			routerStack = [function(req, res, next) {
				...
				if(host.configured) {
					...
					host.logger.warning("Not fully setup yet");
					res.sendStatus(503);
				}
			}];
